...
void loop ( ) {
  ...
  if (mode_select == 0) {
    ...
    if (BT_module.available()>3) {
      char c = BT_module.read();
      while (c != '\n') {
        if ((c >= '0') && (c <= '9')) {
          BT_value *= 10;
          BT_value += (c - '0');
    ...
    yaw_pos = map(yaw_pot_value, 0, 1023, 0, 179) ;
